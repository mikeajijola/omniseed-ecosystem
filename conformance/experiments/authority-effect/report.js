...
record('toctou_resource_version', 'machine', x => x.options.beforeCommit = ({ state }) => state.version++);
record('toctou_pending_operation', 'machine', x => x.options.beforeCommit = ({ pending }) => pending.value = 4);
const report = { schemaVersion: 1, kind: 'experimental-authority-effect', governanceRevision: execFileSync('git', ['rev-parse', 'HEAD'], { encoding: 'utf8' }).trim(), engine: { name: engine.name, version: engine.version }, simulationOnly: true, productionConformance: 'NOT_EVALUATED', standardsFindings: ['STD-PRESS-2026-09-001','STD-PRESS-2026-09-002'], providerPreconditionAudit: 'provider-preconditions.md', scenarios };
