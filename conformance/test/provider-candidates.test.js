...
test("coordinated Provider candidates satisfy the strict contract catalogue", () => {
  ...
  assert.equal(catalogue.candidates.length, 22);
  assert.equal(new Set(catalogue.candidates.map(candidate => candidate.id)).size, 22);
  assert.deepEqual(catalogue.candidates.map(candidate => candidate.issue).sort((a, b) => a - b), [34, 35, 36, 37, 38, 39, 40, 41, 42, 45, 46, 47, 48, 49, 50, 51, 52, 55, 56, 57, 58, 59]);
});
